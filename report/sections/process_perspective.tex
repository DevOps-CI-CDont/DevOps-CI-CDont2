...
For the endpoint counter we gather the metrics by the mean of a middleware, which gets executed after every request. We are using the gin framework in Go, so we are able to register it like so:
